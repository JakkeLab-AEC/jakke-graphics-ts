 ...
 *
 */
export namespace LineEvaluation {
	/**
	...
	export function getIntersection(
		...
		li1: Line,
		fromExtended = false
	): { result: boolean; pt?: Vertex3d; message?: string } {
		if (VectorUtils.isParallelLines(li0, li1))
			return {
				result: false,
				message: "Parallel or same Lines",
			};
		...
		if (Math.abs(det) < TOLERANCE_LINE_EVALUATION) {
			return {
				result: false,
				message: "Det is zero (parallel lines)",
			};
		}
		...
		const ptQ = VectorUtils.add(
			ptLi0EndOnXY,
			VectorUtils.scale(d0PlaneXY, t)
		);
		const ptR = VectorUtils.add(
			ptLi1EndOnXY,
			VectorUtils.scale(d1PlaneXY, u)
		);
		...
		if (dist > TOLERANCE_LINE_EVALUATION)
			return {
				result: false,
				message: "Each points on XY Plane is not same.",
			};

		// Place ptQ on li0
		const tLi0 = VectorUtils.dot(
			VectorUtils.subtract(ptQ, ptLi0StartOnXY),
			d0PlaneXY
		);
		const ratioLi0 =
			VectorUtils.getDist(li0.p1, li0.p0) /
		...
		// Place ptR on li1
		const tLi1 = VectorUtils.dot(
			VectorUtils.subtract(ptR, ptLi1StartOnXY),
			d1PlaneXY
		);
		const ratioLi1 =
			VectorUtils.getDist(li1.p1, li1.p0) /
			VectorUtils.getDist(ptLi1EndOnXY, ptLi1StartOnXY);
		...
		if (VectorUtils.getDist(ptQ2, ptR2) > TOLERANCE_LINE_EVALUATION)
			return {
				result: false,
				message: "Two line's are on skew position.",
			};
		...
		if (fromExtended) {
			...
			if (
				...
				paramPtQ2 <= 1 + TOLERANCE_LINE_EVALUATION &&
				-TOLERANCE_LINE_EVALUATION <= paramPtR2 &&
				paramPtR2 <= 1 + TOLERANCE_LINE_EVALUATION
			) {
				return { result: true, pt: ptQ2 };
			} else {
				return {
	 ...
	 */
	export function getParameterOnLine(
		p0: Vertex3d,
		p1: Vertex3d,
		ptTest: Vertex3d
	): number {
	 ...
	 */
	export function getFootPointOnDirection(
		direction: Vertex3d,
		pt: Vertex3d
	 ...
	 */
	export function getFootPointOnLine(
		line: Line,
		pt: Vertex3d
	): { pt: Vertex3d; t: number } | undefined {
	...
	/**
	 * Calculates the intersection point of a line segment defined by two 3D vertices (`p0` and `p1`)
	 * with a plane parallel to the XY plane at a given Z coordinate.
	 *
	 * @param p0 - The starting vertex of the line segment.
	 * @param p1 - The ending vertex of the line segment.
	 * @param z - The Z coordinate of the plane to intersect with.
	 * @returns The intersection vertex on the plane at Z, or `undefined` if the line is parallel to the plane.
	 */
	export function getIntersectionOnPlaneZ(
		p0: Vertex3d,
		p1: Vertex3d,
		z: number
	): Vertex3d | undefined {
		...
		const x = p0.x + (dx / dz) * (z - p0.z);
		const y = p0.y + (dy / dz) * (z - p0.z);
		return { x: x, y: y, z: z };
	}

	export function createLine2dWithDirection(
		p0: Vertex2d,
		direction: Vertex2d,
		t: number = 1
	): Line2d {
		const pt = VectorUtils.chain({ ...direction, z: 0 })
			.scale(t)
			.add({ ...p0, z: 0 })
			.value();
