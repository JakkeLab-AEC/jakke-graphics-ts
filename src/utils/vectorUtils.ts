...
export namespace VectorUtils {
	 ...
	 */
	export function getBoundingBox2d(pts: (Vertex2d | Vertex3d)[]): {
		min: Vertex2d;
		max: Vertex2d;
	} {
	...
	export function getNormal(
		...
		const p0p2 = VectorUtils.subtract(p2, p0);

		return VectorUtils.chain(p0p1).cross(p0p2).normalize().value();
	}

	export function projectOnPlane(n: Vertex3d, d: Vertex3d): Vertex3d {
		const dProjected = VectorUtils.chain(n)
			.scale(VectorUtils.dot(n, d) / Math.pow(VectorUtils.getSize(n), 2))
			.value();

		return VectorUtils.subtract(d, dProjected);
	}
}
