...
export namespace PointEvaluationUtils {
	/**
	 ...
	 */
	export function pointEvaluationOnLine(
		p0: Vertex3d,
		p1: Vertex3d,
		ptTest: Vertex3d,
	) {
		const direction: Vertex3d = {
			x: p1.x - p0.x,
			y: p1.y - p0.y,
			z: p1.z - p0.z,
		};
		...
		const isAlmostParallel =
			Math.abs(Math.abs(dotResult) - 1) < DIRECTION_TOLERANCE;
	 ...
	 */
	export function getPointOnTrianglePlane(
		line: Line,
		triangle: Triangle,
		onlyWithinTraingle = true,
	): Vertex3d | undefined {
		...
		if (!onlyWithinTraingle) return result;

		const validParams = [u, v, w].every((val) => val >= 0 && val <= 1);
		return validParams ? result : undefined;
	}

	export function projectPointOnTriaglePlane(
		pt: Vertex3d,
		triangle: Triangle,
	) {
		const { p0, p1, p2 } = triangle;
		const n = VectorUtils.getNormal(p0, p1, p2);
		const t = VectorUtils.dot(VectorUtils.subtract(pt, p0), n);
		const ptProjected = VectorUtils.subtract(pt, VectorUtils.scale(n, t));
	...
	function isDetZero(line: Line, triangle: Triangle): boolean {
		...
		const v0v1 = v1.subtract(v0);
		const v0v2 = v2.subtract(v0);
		const d = new Vertex3dWrapped(line.p1)
			.subtract(new Vertex3dWrapped(line.p0))
			.normalized();
		const det = v0v1.dot(d.cross(v0v2));
		return det === 0;
	}
